...
def test_login_view_invalid_credentials(client, profile_factory):
    ...
    data = {"honeypot": "", "email": profile.user.email, "password": "wrongpassword"}
    response = client.post(url, data)
    # assert response.status_code == 302
...
def test_login_view_non_verified(client, profile_factory):
    ...
    assert str(messages[0]) == AccountsMessageManager.resend_verification_email(url)
...
def test_login_view_non_verified_wait(client, profile_factory):
    ...
    with (
        ...
        patch("accounts.views.login.get_minutes_left_before_resend", mock_minutes_left),
    ):
        response = client.post(url, data)
    assert response.status_code == 302  # Should redirect
    assert response.url == reverse("accounts:login")
    messages = list(get_messages(response.wsgi_request))
    assert str(messages[0]) == AccountsMessageManager.resend_email_wait(minutes_left)
